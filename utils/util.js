...
const isAuth = (req, res, next) => {
    ...
    if (authorization) {
        ...
        jwt.verify(token, process.env.JWT_SECRET || 'somethingsecret', async (err, decode) => {
            ...
            // Checking if the user mentioned on the token is verified.
            if (user && !user.isVerified) {
                return res.status(401).send({ message: 'This user has not been verified. Please verify the user using the provided OTP' });
            }
